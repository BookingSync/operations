...
RSpec.describe Operations::Components::OnFailure do
  subject(:component) { described_class.new(on_failure, transaction: transaction, error_reporter: error_reporter) }
  ...
  describe "#call" do
    ...
    context "when no on_failure failures" do
      let(:on_failure) { [->(**) { Dry::Monads::Success({}) }] }
    ...
    it "returns the results of on_failure calls and always successful" do
      expect(call)
        .to be_success
        .and have_attributes(
          component: :operation,
